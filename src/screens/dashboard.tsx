  ...
  return (
    <div className="p-6 max-w-[1280px] mx-auto anim-fade-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-6">
        <StatCard icon="user" label={t("kpi.total_patients")} value={totalPatients} sub={t("kpi.sub_this_week")} tone="primary" trend={[12, 15, 14, 18, 19, 22, totalPatients]} delta={12} />
        <StatCard icon="chart" label={t("kpi.screenings_today")} value={todayScreening} sub={t("kpi.sub_awaiting")} tone="secondary" trend={WEEKLY_SCREENINGS} delta={18} />
        <StatCard icon="bell" label={t("kpi.accuracy")} value={`${accuracy}%`} sub={t("kpi.vs_last_week")} tone="accent" trend={allConfidence.slice(-7)} delta={4} />
        <StatCard icon="warning" label={t("kpi.pneumonia_flagged")} value={highRisk} sub={t("kpi.sub_high_risk")} tone="danger" trend={[2, 3, 2, 4, 3, 5, highRisk]} delta={-8} />
        <StatCard icon="clock" label={t("kpi.pending_decisions")} value={pendingCases} sub={t("kpi.sub_review")} tone="primary" trend={[4, 5, 3, 6, 4, 5, pendingCases]} delta={-5} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-6">
          ...
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[16px] font-bold text-ink">{t("panel.tasks")}</h2>
              <button onClick={() => navigate("/decisions")} className="text-[13px] font-semibold text-primary hover:underline cursor-pointer">{t("task.see_all")}</button>
            </div>
            <div className="space-y-3">
              {TASKS.map((task) => {
                const patient = PATIENTS.find((p) => p.id === task.patientId)
                return (
                  <button key={task.id} onClick={() => navigate(task.patientId ? `/patients/${task.patientId}` : "/decisions")} className="w-full flex items-center gap-3 p-3 rounded-2xl bg-surface-2 hover:bg-primary-soft transition-colors cursor-pointer">
                    <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${task.priority === "high" ? "bg-danger" : task.priority === "medium" ? "bg-accent" : "bg-secondary"}`} />
                    <span className="flex-1 min-w-0">
                      <span className="block text-[14px] font-bold text-ink truncate">{task.title}</span>
                      <span className="block text-[12px] text-muted truncate">{task.body}{patient ? ` · ${patient.name}` : ""}</span>
                    </span>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6">
            <h2 className="text-[16px] font-bold text-ink mb-4">{t("panel.activity")}</h2>
            <div className="space-y-4">
              {ACTIVITIES.slice(0, 4).map((a) => (
                <button key={a.id} onClick={() => a.patientId && navigate(`/patients/${a.patientId}`)} className="w-full flex gap-3 text-left cursor-pointer">
                  <span className="w-9 h-9 rounded-xl bg-surface-2 flex items-center justify-center text-[16px] shrink-0">{a.icon}</span>
                  <div className="min-w-0">
                    <p className="text-[13px] font-semibold text-ink truncate">{a.title}</p>
                    <p className="text-[12px] text-muted truncate">{a.body}</p>
                    <p className="text-[11px] text-faint mt-0.5">{a.time}</p>
                  </div>
                </button>
              ))}
            </div>
